...
## How it works
### Release builds
1. A push affecting (or creating) a production manifest file triggers ```trigger-build.yml``` on the manifest repository.
...
1. A cron-job triggers ```trigger-build.yml``` on the manifest repository.
  - For push-affected development manifests, builds are instead queued to run on next scheduled cron-job.
